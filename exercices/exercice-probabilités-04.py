...
# Si vous activez le code suivant, vous verrez l'erreur "# TypeError: 'tuple' object does not support item assignment"
# my_tuple[0] = 'lorem'
...
cards.remove(card)
alice_cards.append(card)
print("il reste", len(cards), "dans le paquet")
for i in range(len(alice_cards)):
    cards.append(alice_cards.pop())
...
for manches in range(3):
    ...
    count_coeur=0
    for i in range(len(alice_cards)):
        if alice_cards[i][2] == 3:
            alice_victory=True
        if alice_cards[i][0] == 'cœur':
            count_coeur+=1
    if count_coeur>=2:
        alice_victory=True

    if alice_victory:
        print("Alice a gagné son pari")
    else:
        print("Alice a perdu son pari")

    #Alice range ses cartes
    for i in range(len(alice_cards)):
        cards.append(alice_cards.pop())
    ...
    for i in range(len(bob_cards)):
        if bob_cards[i][2] == 7:
            bob_victory=True
        if bob_cards[i][0] == 'pique':
            count_pique+=1
    if count_coeur>=2:
        bob_victory=True

    if bob_victory:
        print("Bob a gagné son pari")
    else:
        print("Bob a perdu son pari")

    #Bob range ses cartes
    for i in range(len(bob_cards)):
        cards.append(bob_cards.pop())